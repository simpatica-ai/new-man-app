...
const Toolbar = ({ editor }: { editor: Editor | null }) => {
  ...
  return (
    <div className="flex items-center gap-2 border border-stone-300 bg-stone-50 p-2 rounded-t-md">
      ...
      <div className="w-px h-6 bg-stone-300 mx-1"></div>
      <button onClick={toggleBulletList} className={`p-2 rounded hover:bg-stone-200 ${isActive('bulletList')}`}>
        <List className="h-4 w-4" />
      </button>
      <button onClick={toggleOrderedList} className={`p-2 rounded hover:bg-stone-200 ${isActive('orderedList')}`}>
