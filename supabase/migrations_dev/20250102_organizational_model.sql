...
-- Split into two ALTER statements to handle type mismatch with virtues.id (BIGINT not UUID)
...
-- Add current_virtue_id separately with correct type (BIGINT to match virtues.id)
ALTER TABLE public.profiles 
  ADD COLUMN IF NOT EXISTS current_virtue_id BIGINT REFERENCES public.virtues(id) ON DELETE SET NULL;
...
-- FIXED: Removed recursive subquery that caused infinite recursion
-- The organization membership check has been simplified to avoid querying profiles within profiles policy
...
CREATE POLICY "Users can view accessible profiles" ON public.profiles
  FOR SELECT USING (
    ...
    -- Using function to avoid recursive query
...
-- FIXED: Using function to avoid recursive query
...
BEGIN
  ...
  IF user_count > 0 THEN
    ...
    INSERT INTO public.organizations (
      ...
      'basic', 
      'active',
      40, -- Standard limit (constraint enforces max 40)
