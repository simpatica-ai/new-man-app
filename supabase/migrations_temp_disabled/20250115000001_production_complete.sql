-- ============================================================================
-- COMPLETE PRODUCTION MIGRATION SCRIPT V2
-- Date: 2025-01-14
-- 
...
-- 8. Add organization invitations system (NEW)
-- 9. Add user_limit column to organizations (NEW)
-- ============================================================================
...
-- ============================================================================
-- STEP 2: CREATE ORGANIZATIONS TABLE (MUST COME BEFORE PROFILES UPDATE)
-- ============================================================================
...
-- ============================================================================
-- STEP 3: FIX PROFILES SCHEMA MISMATCH (AFTER ORGANIZATIONS EXISTS)
-- ============================================================================
...
-- RLS Policies for work product reports (drop and recreate for idempotency)
DROP POLICY IF EXISTS "Users can view their own work product reports" ON public.work_product_reports;
CREATE POLICY "Users can view their own work product reports" ON public.work_product_reports
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can create their own work product reports" ON public.work_product_reports;
CREATE POLICY "Users can create their own work product reports" ON public.work_product_reports
...
-- Add updated_at trigger (drop and recreate for idempotency)
DROP TRIGGER IF EXISTS update_work_product_reports_updated_at ON public.work_product_reports;
...
-- Note: user_virtue_stage_memos table will be created in a later migration
-- Skipping ALTER TABLE for now as table doesn't exist yet
-- This will be handled when the table is created

-- Add updated_at to user_virtue_stage_memos if it exists
DO $
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_virtue_stage_memos') THEN
    ALTER TABLE public.user_virtue_stage_memos 
    ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    
    -- Add trigger (drop and recreate for idempotency)
...
BEGIN
  ...
  -- Check each role is valid (updated to include org-prefixed and individual roles)
  FOR i IN 1..array_length(user_roles, 1) LOOP
    IF user_roles[i] NOT IN (
      'sys-admin', 'org-admin', 'org-coach', 'org-therapist', 'org-practitioner',
      'ind-sponsor', 'ind-practitioner',  -- Individual roles
      'coach', 'practitioner', 'therapist',  -- Legacy roles
...
SET roles = CASE 
  ...
  WHEN role = 'sponsor' THEN ARRAY['org-coach'] 
  WHEN role = 'user' THEN ARRAY['org-practitioner']
  WHEN role = 'coach' THEN ARRAY['org-coach']
  WHEN role = 'therapist' THEN ARRAY['org-therapist']
  WHEN role = 'practitioner' THEN ARRAY['org-practitioner']
  WHEN roles IS NULL AND role IS NOT NULL THEN ARRAY[role]
  ELSE COALESCE(roles, ARRAY['org-practitioner'])
END
WHERE roles IS NULL OR array_length(roles, 1) IS NULL;

-- ============================================================================
-- STEP 7: ORGANIZATION REQUEST SYSTEM
-- ============================================================================
...
DROP POLICY IF EXISTS "System admins can view all organization requests" ON public.organization_requests;
...
DROP POLICY IF EXISTS "System admins can update organization requests" ON public.organization_requests;
...
DROP POLICY IF EXISTS "Anyone can create organization requests" ON public.organization_requests;
...
DROP POLICY IF EXISTS "Only sys-admins can delete organizations" ON public.organizations;
CREATE POLICY "Only sys-admins can delete organizations" ON public.organizations
  FOR DELETE USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE id = auth.uid() 
...
-- Add updated_at trigger (drop and recreate for idempotency)
DROP TRIGGER IF EXISTS update_organization_requests_updated_at ON public.organization_requests;
CREATE TRIGGER update_organization_requests_updated_at 
  BEFORE UPDATE ON public.organization_requests
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- ============================================================================
-- STEP 8: ORGANIZATION INVITATIONS SYSTEM (NEW)
-- ============================================================================
...
CREATE POLICY "Org admins can create invitations" ON public.organization_invitations
    FOR INSERT WITH CHECK (
        organization_id IN (
            ...
            FROM public.profiles 
            WHERE id = auth.uid() 
            AND 'org-admin' = ANY(roles)
        )
    );

-- Policy: Org admins can update invitations (for resending, etc.)
...
-- Policy: Anyone can view invitations by token (for accepting invitations)
...
-- Add updated_at trigger for organization_invitations (drop and recreate for idempotency)
...
-- ============================================================================
-- STEP 9: ADD USER_LIMIT COLUMN TO ORGANIZATIONS (NEW)
-- ============================================================================
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN QUERY SELECT false, NULL::TEXT, 'Invalid or expired invitation token';
    RETURN;
  END IF;
  ...
  IF NOT FOUND THEN
    RETURN QUERY SELECT false, NULL::TEXT, 'User profile not found';
    RETURN;
  END IF;
  
  -- Check if user is already in an organization
  IF user_profile.organization_id IS NOT NULL THEN
    RETURN QUERY SELECT false, NULL::TEXT, 'User is already a member of an organization';
    RETURN;
  END IF;
  
  -- Update user profile with organization and roles
  UPDATE public.profiles 
  SET 
...
EXCEPTION
  WHEN OTHERS THEN
    RETURN QUERY SELECT 
      false, 
      NULL::TEXT,
      format('Error accepting invitation: %s', SQLERRM);
END;
...
BEGIN
  ...
  -- Step 2: Delete organization invitations
  DELETE FROM public.organization_invitations 
  WHERE organization_id = org_id;
  
  -- Step 3: Delete organization requests related to this org
  DELETE FROM public.organization_requests 
  WHERE organization_name = org_record.name;
  
  -- Step 4: Remove organization_id from profiles first (break the foreign key reference)
  UPDATE public.profiles 
  SET organization_id = NULL 
  WHERE organization_id = org_id;
  
  -- Step 5: Now delete the organization (no more foreign key references)
  DELETE FROM public.organizations 
  WHERE id = org_id;
  
  -- Step 6: Delete the users (now that organization is gone)
...
DROP POLICY IF EXISTS "Sys admins can manage pending org admins" ON public.pending_org_admins;
CREATE POLICY "Sys admins can manage pending org admins" ON public.pending_org_admins
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE id = auth.uid() 
...
-- Create trigger to run after user signup (drop and recreate for idempotency)
DROP TRIGGER IF EXISTS assign_org_admin_on_signup ON auth.users;
CREATE TRIGGER assign_org_admin_on_signup
...
WHERE EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_name = 'approve_organization_request')
  ...
  AND EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_name = 'accept_organization_invitation');
...
COMMENT ON FUNCTION accept_organization_invitation(TEXT, UUID) IS 'Accepts an organization invitation and assigns user to organization';

-- If you see all success messages above, the migration completed successfully!
-- Your application should now have all organizational features available including:
-- - Organization requests and approval system
